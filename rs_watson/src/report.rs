...
impl Report {
    pub fn from_frames(frames: &[Frame]) -> Self {
        ...
        let total = projects
            .iter()
            .fold(Duration::zero(), |acc, p| acc + p.total);
...
mod tests {
    ...
    fn multiple_frames_same_project_totals_sum() {
        let r = Report::from_frames(&[frame("backend", &[], 9, 10), frame("backend", &[], 11, 13)]);
        assert_eq!(r.projects.len(), 1);
    ...
    fn grand_total_equals_sum_of_project_totals() {
        let r = Report::from_frames(&[frame("a", &[], 9, 10), frame("b", &[], 10, 12)]);
